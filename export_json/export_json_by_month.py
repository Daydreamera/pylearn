...
data_path = r'\\10.20.202.51\Datacopy\标准化试用数据\基础数据'
...
def export_do(data_apth_var, table_name_var, date_begin_var, date_end_var, export_flg_1_var):
    ...
    for index, row in sql2_pre.iterrows():
        ...
        if export_flg_1_var == 'split':
            df.to_json(path_or_buf="%s_%s_%s.json" % (
                data_apth_var + """/""" + table_name_var, str(date_begin_var.year), str(date_begin_var.month)),
...
def main(table_name_var):
    ...
    for temp_table in table_list:
        ...
        file_path = '%s%s/%s' % (data_path, temp_path_var, temp_table)
        ...
        date_begin_monthend = date_begin.replace(month=date_begin.month,
                                                 day=calendar.monthrange(date_begin.year, date_begin.month)[1], hour=23,
                                                 minute=59, second=59)
        ...
        if check_date_constr_df["date_constr"][0] is not None:
            if date_begin_yearend - date_begin > cur_date - date_begin:
                ...
                temp_date_end = date_begin_monthend
                while (1 == 1):
                    if not os.path.exists(file_path + '/' + str(temp_date_start.year)):
                        os.mkdir(file_path + '/' + str(temp_date_start.year))
                        while (1 == 1):
                            if temp_date_end > cur_date:
                                export_do(file_path, temp_table, temp_date_start, cur_date, 'split')
                                break
                            export_do(file_path + '/' + str(temp_date_start.year), temp_table, temp_date_start,
                                      temp_date_end, 'split')
                            temp_date_start = temp_date_end + datetime.timedelta(seconds=1)
                            # temp_date_end = temp_date_end + datetime.timedelta(days=365)
                            if temp_date_start.year > temp_date_end.year:
                                temp_date_end = temp_date_end + datetime.timedelta(
                                    days=calendar.monthrange(temp_date_start.year, temp_date_start.month)[1])
                                break
                            temp_date_end = temp_date_end + datetime.timedelta(
                                days=calendar.monthrange(temp_date_start.year, temp_date_start.month)[1])
